...
$direcurl = filter_input(INPUT_POST, 'direcurl')
...
$statement = 'INSERT INTO objeto_aprendizaje (nombre,descripcion,id_usuario,institucion,palabras_clave,tamanio,ruta,materia, tipo_repo,descarga,Privado,Publico, direcurl) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)';
$consulta = $conexion->prepare($statement);
if($repo == "PUB"){
    $cadena=array($nombre, $descripcion, $id_usuario, $institucion, $palabras_clave, $_FILES["o_aprendizaje"]["size"], $target_file, consultar_materiaxid($cbx_materia), 1,0,"SI","SI", $direcurl);
}

if($repo == "PRI"){
    $cadena=array($nombre, $descripcion, $id_usuario, $institucion, $palabras_clave, $_FILES["o_aprendizaje"]["size"], $target_file, consultar_materiaxid($cbx_materia), 1,0,"SI","NO",$direcurl);
}
